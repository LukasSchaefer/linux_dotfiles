...
for m in $(xrandr --query | grep " connected" | cut -d" " -f1); do
    if [[ $m != "eDP"* ]] ; then
        DP_connected=$m
        DP_resolution=$(xrandr -q | awk "/$m/{getline; print}" | awk 'NR==1{print $1}')
        echo "$m $DP_resolution"
    else
        eDP_connected=$m
    fi
done
if [ "$DP_connected" != "none" ]; then
    ...
    bash ~/.screenlayout/dual_screen_left_portrait.sh $eDP_connected $DP_connected $DP_resolution
else
